...
public class AgentHider : Agent
{
    ...
    public override void Initialize()
    {
        ...
        m_LateralSpeed = .7f;
        m_ForwardSpeed = .8f;
        m_jumpPotential = 1.3f;
    	agentRb = GetComponent<Rigidbody>();
    	agentRb.maxAngularVelocity = 280;	
    }

    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
    	if (envController.InStateOfHide == false) { //decay reward for only seeking period
    		var r = rewfunc.DecayCompetitiveR(envController.GetCurrentSHtimeinterval(), "upend");
    		AddReward(r);
    	}
	...
	{
        var r = - 1f * rewfunc.QuadCompetitiveR(envController.GetCurrentSHtimeinterval(), "upbegin") - 0.8f;
		AddReward(r);
        EndEpisode();
	}
